...
#########################################################################################################
#計算量の見積もり
#時間計算量：O(26nm)=O(nm), n=辞書内の単語の数, m=辞書内の単語の長さ, 26はアルファベットの個数で固定長
#空間計算量：O(26) = O(1), 26はアルファベットの個数で固定長

#実行時間の比較 - "zo olo calgi" => "zoological" (zは後半のアルファベット∧文字列の長さが長いため)
#①頭文字と文字列長による絞りこみ：0.02116870880126953
#②文字列長のみによる絞りこみ：0.04760551452636719
#③頭文字のみによる絞りこみ：0.10075521469116211
#④絞りこみなし：0.2315208911895752

#今回は文字列の長さが長めの単語を選んだため，同じ文字数の単語が少なく，頭文字による絞りこみよりも，文字列の長さによる絞りこみの方が効いていた．
#絞り込みなしの場合と①を比べると，今回の単語では約10倍の実行時間の差があり，かなり実行時間と計算量を減らすことができていたのが分かった．
#########################################################################################################
