...
class Cache:
  ...
  def access_page(self, url, contents):
    ###########################
    # Write your code here :) #
    if len(self.preserve[url])==0:
        if len(self.order) >= self.n:
            del self.preserve[self.order.pop()]
        self.preserve[url] = contents
    else:
        self.order.remove(url)
    self.order.appendleft(url)
    #print('successfully renewed cache, ', url, contents)
    ###########################
    pass
  ...
  def get_pages(self):
    ###########################
    # Write your code here :) #
    #print('recently accessed pages are, ', self.cache)
