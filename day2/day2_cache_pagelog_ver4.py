class Cache:
    ...
    def access_page(self, url, contents):
        ...
        if url not in self.preserved:
            ...
            self.preserved[url] = [contents, "head", "tail"] #前後のページは0で初期化
            self.length += 1
            if self.length == 1:
                self.head = url
                self.tail = url
                ...
                if self.length > self.n:
                    next_head = self.preserved[self.head][2]
                    del self.preserved[self.head]
                    self.head = next_head
                    self.preserved[self.head][1] = "head" #一番最初になったら0に初期化
                    self.length -= 1
                self.preserved[self.tail][2] = url
        else:
            #訪れたurlがpreserved内に存在していたら，その部分を抜き取って前後のpreservedを繋げるようにする．
            #preservedの先頭だった場合，場合分けが必要
            if self.preserved[url][1] == "head":
                self.head = self.preserved[self.head][2]
                self.preserved[self.head][1] = "head" #一番最初になったら0に初期化
            #preservedの末尾だった場合も，場合分けが必要(なにもしない)
            elif self.preserved[url][2] == "tail":
                return
            else:
        ...
        self.preserved[url][1] = self.tail
        self.preserved[url][2] = "tail" #最後尾に来たら次に訪れたurlを0に初期化
        self.tail = url
        ###########################
        pass
    ...
    def get_pages(self):
        ...
        if len(self.preserved) != 0:
            cache_urls.append(self.tail)
            before_url = self.preserved[self.tail][1]
            while (before_url != "head"):
